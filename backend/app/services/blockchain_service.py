...
def verify_system_integrity(file_id: int, user_id: int) -> Dict[str, Any]:
    """
    Performs multi-way cross-system integrity verification between DB, IPFS, and Blockchain.
    """
    from app.services.file_service import get_file_record
    from app.services.integrity_service import calculate_bytes_sha256
    from app.services.ipfs_service import get_bytes_from_ipfs
    ...
    if file_record["owner_id"] != user_id:
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT id FROM shares WHERE file_id = %s AND receiver_id = %s AND status = 'active'",
                    (file_id, user_id)
                )
                share = cursor.fetchone()
                if not share:
                    raise PermissionError("Unauthorized access to file integrity verification.")
        finally:
            conn.close()

    stored_hash = file_record.get("sha256_hash", "").lower()
    ipfs_cid = file_record.get("ipfs_cid", "")

    # IPFS Payload SHA-256 Check
    ciphertext_bytes = get_bytes_from_ipfs(ipfs_cid)
    computed_ipfs_hash = calculate_bytes_sha256(ciphertext_bytes)
    ipfs_hash_verified = (computed_ipfs_hash == stored_hash)

    # Blockchain Cross-Verification Check
    blockchain_recorded = False
    blockchain_hash = None
    blockchain_hash_verified = True
    blockchain_cid_verified = True

    try:
        if is_file_registered(file_id):
            bc_meta = get_file_metadata(file_id)
            blockchain_recorded = True
            blockchain_hash = bc_meta.get("sha256_hash", "").lower()
            blockchain_hash_verified = (blockchain_hash == stored_hash)
            blockchain_cid_verified = (bc_meta.get("ipfs_cid") == ipfs_cid)
    except Exception as e:
        logger.warning(f"Blockchain verification check skipped: {e}")

    overall = ipfs_hash_verified and blockchain_hash_verified and blockchain_cid_verified

    return {
        "success": True,
        "file_id": file_id,
        "db_sha256": stored_hash,
        "ipfs_sha256": computed_ipfs_hash,
        "ipfs_integrity_verified": ipfs_hash_verified,
        "blockchain_recorded": blockchain_recorded,
        "blockchain_sha256": blockchain_hash,
        "blockchain_hash_verified": blockchain_hash_verified,
        "blockchain_cid_verified": blockchain_cid_verified,
        "status": "VERIFIED" if overall else "TAMPERED"
    }
